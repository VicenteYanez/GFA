...
while ok in ['n', 'N']:
    ...
    if stndr in ['y', 'Y', 's', 'S', 'O', 'o']:
        ...
        # polinomial part
        ok_pl = input(color.BOLD+"The stndr value for polinomial function is 1\
, do you want to change it? y/n "+color.END)
        print(" ")
        if ok_pl in ['y', 'Y', 's', 'S', 'O', 'o']:
            pl = int(input(color.BOLD+color.PURPLE+"Order for the polimolial \
function: "+color.END))
            print(" ")
        else:
            pl = 1
...
"+color.END))
                print(" ")
                for g in range(n_hs):
                    hsd = float(input(color.BOLD+color.PURPLE+"Date for the \
"+str(g+1)+" heaviside jump: "+color.END))
                    print(" ")
                    hs.append(hsd)
            else:
                print(color.BOLD+color.RED+"The last values will be used for \
jumps "+str(hs)+color.END)
                print(" ")
        else:
            hs = [2010.15, 2014.24832]

        # logs
        ok_lb = input(color.BOLD+"the stndr number of log curves is 2 [2010.01\
, 2014.24832] [1, 1], do you want to change it? y/n "+color.END)
        print(" ")
        if ok_lb in ['y', 'Y', 's', 'S', 'O', 'o']:
            ok_lb2 = input(color.BOLD+"do you want to select the dates? y/n,\
if n, last dates will be used "+color.END)
            print(" ")
            if ok_lb2 in ['y', 'Y', 's', 'S', 'O', 'o']:
                lb = []
                ls = []
                n_lb = int(input(color.BOLD+color.PURPLE+"Number of log curves\
: "+color.END))
                print(" ")
                for g in range(n_lb):
                    lbd = float(input(color.BOLD+color.PURPLE+"Date for the \
"+str(g+1)+" log curve: "+color.END))
                    print(" ")
                    lb.append(lbd)

                    lsd = float(input(color.BOLD+color.PURPLE+"Scale for the \
"+str(g+1)+" log curve: "+color.END))
                    print(" ")
                    ls.append(lsd)
            else:
                print(color.BOLD+color.RED+"The last values will be used for\
log curves"+str(lb)+color.END)
                print(" ")
...
"+str(ls)+color.END)
        ...
        # fourier
        ok_fp = input(color.BOLD+"the stndr number of Fourier periods is 1 \
with a period of 1 year, do you want to change it? y/n "+color.END)
        print(" ")
...
ok_no_f = input(color.BOLD+"Do you want to compute a solution with no seasonal\
component? y/n "+color.END)
print(" ")
